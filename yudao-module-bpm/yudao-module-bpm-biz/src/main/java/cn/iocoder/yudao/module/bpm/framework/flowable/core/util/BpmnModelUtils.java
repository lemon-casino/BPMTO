...
public class BpmnModelUtils {
    ...
    public static void addCopySelf(Boolean copySelf, FlowElement flowElement) {
        addExtensionElement(flowElement, COPY_SELF,
                ObjUtil.isNotNull(copySelf) ? copySelf.toString() : Boolean.FALSE.toString());
    }

    public static Boolean parseCopySelf(BpmnModel bpmnModel, String flowElementId) {
        FlowElement flowElement = getFlowElementById(bpmnModel, flowElementId);
        return flowElement != null && parseCopySelf(flowElement);
    }

    public static Boolean parseCopySelf(FlowElement flowElement) {
        if (flowElement == null) {
            log.debug("[parseCopySelf][flowElement为空，返回false]");
            return false;
        }
        
        // 获取扩展元素
        Map<String, List<ExtensionElement>> extensionElements = flowElement.getExtensionElements();
        if (extensionElements == null || !extensionElements.containsKey(COPY_SELF)) {
            log.debug("[parseCopySelf][节点({})不包含copySelf扩展元素，返回false]", flowElement.getId());
            return false;
        }
        
        List<ExtensionElement> elements = extensionElements.get(COPY_SELF);
        if (CollUtil.isEmpty(elements)) {
            log.debug("[parseCopySelf][节点({})的copySelf扩展元素为空列表，返回false]", flowElement.getId());
            return false;
        }
        
        String value = elements.get(0).getElementText();
        Boolean result = Convert.toBool(value, false);
        log.debug("[parseCopySelf][节点({})的copySelf扩展元素值为{}，解析结果为{}]", 
                 flowElement.getId(), value, result);
        return result;
    }


    public static void addWorkTimeEnable(Boolean workTimeEnable, FlowElement flowElement) {
        addExtensionElement(flowElement, USER_TASK_WORK_TIME_ENABLE,
    ...
    public static Boolean parseWorkTimeEnable(FlowElement flowElement) {
        ...
        List<ExtensionElement> extensionElements = flowElement.getExtensionElements().get(USER_TASK_WORK_TIME_ENABLE);
        if (CollUtil.isEmpty(extensionElements)) {
            return false;
        }
        return Convert.toBool(extensionElements.get(0).getElementText(), false);
     ...
     * @return 任务超时跳转节点 ID
     */
    public static String parseTimeoutReturnNodeId(FlowElement flowElement) {
        return parseExtensionElement(flowElement, USER_TASK_TIMEOUT_JUMP_TASK_ID);
    }
     ...
     *
     * @param userTask 任务节点
     * @param returnNodeId 超时跳转节点 ID
     */
    public static void addTimeoutReturnNodeId(UserTask userTask, String returnNodeId) {
        addExtensionElement(userTask, USER_TASK_TIMEOUT_JUMP_TASK_ID, returnNodeId);
    }
     ...
     * @return 工作时间类型
     */
    public static Integer parseWorkTimeType(FlowElement flowElement) {
        return NumberUtils.parseInt(parseExtensionElement(flowElement, BpmnModelConstants.USER_TASK_WORK_TIME_TYPE));
    }
     ...
     * @return 是否需要工作时间计算
     */
    public static boolean needWorkTimeCalculation(FlowElement flowElement) {
        Boolean workTimeEnable = parseWorkTimeEnable(flowElement);
        Integer workTimeType = parseWorkTimeType(flowElement);
        return Boolean.TRUE.equals(workTimeEnable) && workTimeType != null;
    }
     ...
     *
     * @param flowElement 任务节点
     * @return 工作时间配置，包含是否启用和工作时间类型
     */
    public static WorkTimeConfig getWorkTimeConfig(FlowElement flowElement) {
        Boolean workTimeEnable = parseWorkTimeEnable(flowElement);
        Integer workTimeType = parseWorkTimeType(flowElement);
        return new WorkTimeConfig(workTimeEnable, workTimeType);
    }
    ...
    public static class WorkTimeConfig {
        ...
        public WorkTimeConfig(Boolean enable, Integer type) {
            this.enable = enable;
            this.type = type;
        }
     ...
     * 判断是否为网关节点
     */
    public static boolean isGateway(FlowElement element) {
        return element instanceof Gateway;
    }

    /**
     * 判断是否为用户任务
     */
    public static boolean isUserTask(FlowElement element) {
     ...
     * 查找对应的聚合网关
     */
    public static FlowElement findCorrespondingJoinGateway(BpmnModel bpmnModel, Gateway splitGateway) {
        // 简化实现，实际需要根据网关类型和流程结构来查找
        String gatewayType = splitGateway.getClass().getSimpleName();
        String joinGatewayId = splitGateway.getId() + "_join";

        return getFlowElementById(bpmnModel, joinGatewayId);
    }

    /**
     * 判断任务是否在指定分支中
     */
    public static boolean isTaskInBranch(BpmnModel bpmnModel, String taskDefinitionKey, SequenceFlow branch) {
        // 简化实现，实际需要分析分支路径
        return true;
    }
     ...
     * 获取可达的任务Keys
     */
    public static Set<String> getReachableTaskKeys(BpmnModel bpmnModel, FlowElement startElement,
                                                   List<SequenceFlow> outgoingFlows) {
        Set<String> taskKeys = new HashSet<>();
        Set<String> visited = new HashSet<>();

        for (SequenceFlow flow : outgoingFlows) {
            traverseForTaskKeys(flow.getTargetFlowElement(), taskKeys, visited);
        }
     ...
     * 遍历获取任务Keys
     */
    private static void traverseForTaskKeys(FlowElement element, Set<String> taskKeys, Set<String> visited) {
        ...
        // 继续遍历
        List<SequenceFlow> outgoingFlows = getElementOutgoingFlows(element);
        for (SequenceFlow flow : outgoingFlows) {
            traverseForTaskKeys(flow.getTargetFlowElement(), taskKeys, visited);
        }
    }
     ...
     * @return 多实例类型（BpmUserTaskApproveMethodEnum 中的类型值）；如果不是多实例节点或无法识别类型则返回 null
     */
    public static Integer getMultiInstanceType(FlowElement flowElement) {
        if (!(flowElement instanceof UserTask)) {
            return null;
        }
        ...
        // 获取节点的审批方式扩展属性
        String approveMethod = parseExtensionElement(userTask, BpmnModelConstants.USER_TASK_APPROVE_METHOD);
        if (StrUtil.isBlank(approveMethod)) {
            return null;
        }
        
        // 转换为整数类型并返回
        return Integer.parseInt(approveMethod);
    }

    public static List<UserTask> getFlowElementsOfType(ParallelGateway parallelGateway, Class<UserTask> userTaskClass) {
        Collection<FlowElement> flowElements = parallelGateway.getParentContainer().getFlowElements();
        return flowElements.stream()
                .filter(userTaskClass::isInstance)
                .map(userTaskClass::cast)
                .collect(Collectors.toList());
    }

    public static List<FlowElement>  getPreviousFlowElementById(BpmnModel bpmnModel, String taskDefinitionKey) {
        // 1. 获取当前节点
        FlowElement currentElement = getFlowElementById(bpmnModel, taskDefinitionKey);
        if (currentElement == null) {
            return null;
        }
        ...
        if (currentElement instanceof FlowNode) {
            incomingFlows.addAll(((FlowNode) currentElement).getIncomingFlows());
        }

        // 3. 获取去重的流入连线的源节点
        Set<FlowElement> sourceElements = new HashSet<>();
        for (SequenceFlow flow : incomingFlows) {
            sourceElements.add(flow.getSourceFlowElement());
        }

        // 4. 返回去重的流入连线的源节点
        return new ArrayList<>(sourceElements);
    }
     ...
     * @return 工作时间配置的详细信息
     */
    public static String validateAndLogWorkTimeConfig(FlowElement flowElement, String taskId) {
        if (flowElement == null) {
            return String.format("任务(%s)的FlowElement为空", taskId);
        }
